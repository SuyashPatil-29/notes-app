          ...
          {result.count > 0 ? (
            <>
              <p className="font-semibold text-foreground">
                Found {result.count} note{result.count !== 1 ? "s" : ""}{" "}
              ...
              <div className="space-y-2">
                {result.results.map((note: any, idx: number) => (
                  <div
                    key={idx}
                    className="p-2 bg-muted/50 rounded border space-y-2"
                  >
                    <div>
      ...
      return (
        <div className="space-y-2 text-sm">
          {result.count > 0 ? (
            <>
              <p className="font-semibold text-foreground">
                Found {result.count} notebook{result.count !== 1 ? "s" : ""}
              </p>
              <div className="space-y-1">
                {result.notebooks.map((notebook: any, idx: number) => (
                  <div key={idx} className="p-2 bg-muted/50 rounded border">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {notebook.chapterCount} chapter
                      {notebook.chapterCount !== 1 ? "s" : ""} • Updated:{" "}
                      {notebook.updatedAt}
                    </p>
                  </div>
                ))}
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="p-3 bg-muted/50 rounded border max-h-96 overflow-y-auto">
            <p className="text-xs whitespace-pre-wrap">
              {getPreviewText(result.content || "", 500)}
            </p>
          </div>
        </div>
      );
            ...
            <>
              ...
              <div className="p-3 bg-muted/50 rounded border space-y-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Size: {result.contentSize} characters • Created:{" "}
                    {result.createdAt}
                  </p>
                </div>
                {onNavigateToNote &&
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    <p className="text-xs text-muted-foreground">
                      ...
                      {result.fromNotebook} → {result.fromChapter}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      <span className="font-semibold">To:</span>{" "}
                      {result.toNotebook} → {result.toChapter}
                    </p>
                  </div>
                </div>
                {onNavigateToNote &&
              ...
              <div className="p-3 bg-muted/50 rounded border">
                ...
                <div className="mt-2 space-y-1">
                  ...
                  <p className="text-xs text-primary mt-2">
                    📦 Moved with {result.notesCount} note
                    {result.notesCount !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
            </>
          ...
          {result.success ? (
            <>
              ...
              <div className="p-3 bg-muted/50 rounded border space-y-2">
                <div>
                  <p className="text-xs text-muted-foreground">
                    <span className="line-through">{result.oldName}</span> →{" "}
            ...
            <>
              ...
              <div className="p-3 bg-muted/50 rounded border space-y-2">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    New size: {result.contentSize} characters • Updated:{" "}
                    {result.updatedAt}
                  </p>
                </div>
                {onNavigateToNote &&
...
export function RightSidebarContent() {
  ...
  const allNotes = useMemo(() => {
    if (!notebooks) return [];
    return notebooks.flatMap(
      (notebook: Notebook) =>
        notebook.chapters?.flatMap(
          (chapter: any) =>
  ...
  const { data: fetchedApiKeyStatus } = useQuery<ApiKeyStatus>({
    ...
    queryFn: async () => {
      ...
      let status = {
        ...
        anthropic: individualProviders.anthropic || false,
        google: individualProviders.google || false,
      };

      // If in organization context, check for organization API keys (they take priority)
      if (activeOrg?.id) {
        try {
          const orgResponse = await api.get(
            `/organizations/${activeOrg.id}/api-credentials`
          );
          ...
          orgCredentials.forEach((cred: any) => {
            if (
              cred.hasKey &&
              status[cred.provider as keyof ApiKeyStatus] !== undefined
            ) {
          ...
          if (orgError?.response?.status === 403) {
            ...
            console.warn("[API Key Status] Organization not found or no API credentials configured");
          }
        }
      }

      console.log("[API Key Status] Final status:", status);
  ...
  const apiKeyStatus = fetchedApiKeyStatus || {
    openai: false,
    anthropic: false,
    google: false,
  };
  ...
  const hasSelectedApiKey =
    apiKeyStatus[selectedProvider as keyof ApiKeyStatus] || false;

  const chatFetch = React.useCallback(
  ...
  const handleMentionedNotesChange = React.useCallback(
    (notes: MentionedNote[]) => {
  ...
  // Navigate to a note
  const handleNavigateToNote = React.useCallback(
    (notebookId: string, chapterId: string, noteId: string) => {
      if (!notebookId || !chapterId || !noteId) {
        toast.error(
          "Cannot navigate: Missing notebook or chapter information",
          { duration: 3000 }
        );
        return;
      }
  ...
  const handleSubmitWithFiles = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mentionedNotes.length > 0) {
      const notesContext = mentionedNotes
        .map(
          (note) =>
        ...
        if (
          ...
          if (!processedNoteIds.current.has(operationKey)) {
            ...
            if (
              ...
              const notebooks = queryClient.getQueryData([
                "userNotebooks",
              ]) as any[];
              if (notebooks) {
                for (const notebook of notebooks) {
                  for (const chapter of notebook.chapters || []) {
                    const note = chapter.notes?.find(
                      (n: any) => n.id === noteId
                    );
                    if (note) {
        ...
        if (part.type === "tool-invocation" && "toolInvocation" in part) {
          ...
          if (isModifyingTool && hasSuccessResult) {
            ...
            if (!processedNoteIds.current.has(operationKey)) {
              ...
              toast.promise(Promise.all(queriesToRefetch), messages);
            }
          }
        }
      });
                    ...
                    <MessageContent>
                      <div className="space-y-2">
                        ...
                        <div className="text-sm">
                          {renderMessageWithMentions(mainMessage)}
                        </div>
                      </div>
                    </MessageContent>
                  </Message>
                      ...
                      <Message from={role}>
                        ...
                        <MessageContent>
                          {toolParts.map((part, index) => {
                            // Type guard to ensure we have a tool invocation part
        ...
        <div className="p-6 pt-4 border-t bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60">
          <PromptInput onSubmit={handleSubmitWithFiles} className="shadow-lg">
            <MentionTagsInput
              ...
              placeholder={
                hasSelectedApiKey
                  ? "Ask me anything... (Type @ to mention notes)"
                  : "Set up API keys in settings to start chatting..."
              }
              disabled={
                status === "streaming" ||
                status === "submitted" ||
